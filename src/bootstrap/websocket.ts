 ...
 *   - 独立进程避免端口冲突
 *   - 跨 Worker 的消息通过 Redis Pub/Sub 分发
 *   - 定时器等仅在 Master 进程运行
 */
...
import { createCrossProcessEventBus, broadcast } from '#bootstrap/events';
...
export async function createStandaloneWebSocketServer(port?: number): Promise<http.Server> {
  ...
  // 初始化跨进程事件总线（用于广播消息）
  await createCrossProcessEventBus({ channel: 'ws:broadcast' });

  // 订阅跨进程消息并分发给本地连接的客户端
  broadcast.subscribe('ws:broadcast', (data: { channel: string; payload: any; excludePid?: number }) => {
...
export async function broadcastToRoom(room: string, message: any) {
  broadcast.publish('ws:broadcast', {
    channel: room,
    payload: message,
    timestamp: Date.now(),
  });
